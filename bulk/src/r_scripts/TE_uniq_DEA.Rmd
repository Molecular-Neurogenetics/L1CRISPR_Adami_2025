...
```{r}
candidate_L1s <- c("L1PA2_dup2271", # ELAPOR
                   "L1PA3_dup9610", "L1PA2_dup4399", #"WDR72"
                   "L1PA3_dup10384", "L1PA2_dup4791", # PLCB1,
                   "L1PA2_dup623", #PPP1R1C
                   "L1PA2_dup3884", "L1PA2_dup3885", # SLCO1A2 
                   "L1PA3_dup1045", #REG1A
                   "L1PA3_dup4900", # DYNC1I1
                   "L1PA3_dup621", # CFHR3
                   "L1PA2_dup2180") # POU6F2
  
candidate_te_counts_norm <- FL_L1PA_norm[candidate_L1s,]
candidate_L1_boxplots <- list()
for(i in names(samplesheet_comparisons)){
  candidate_L1_boxplots[[i]] <- list()
  for(L1 in candidate_L1s){
    coldata <- samplesheet_comparisons[[i]]
    group_candidate_te_counts_norm_melt <- reshape2::melt(candidate_te_counts_norm[L1, coldata$samples])
    group_candidate_te_counts_norm_melt <- merge(group_candidate_te_counts_norm_melt, coldata, by.x="variable", by.y="samples")
    max_lim <- max(group_candidate_te_counts_norm_melt$value) + mean(group_candidate_te_counts_norm_melt$value)/2
    
    padj_L1s <- te_res_list_df[[i]][candidate_L1s, "padj", drop=F]
    padj_L1s$group1 <- unique(group_candidate_te_counts_norm_melt$condition)[1]
    padj_L1s$group2 <- unique(group_candidate_te_counts_norm_melt$condition)[2]
    padj_L1s$padj <- format(padj_L1s$padj, digits = 2)
    padj_L1s_split <- split(padj_L1s, rownames(padj_L1s))
    candidate_L1_boxplots[[i]][[L1]] <-
      ggplot(data = group_candidate_te_counts_norm_melt, aes(x=condition, y=value)) +
      geom_boxplot(alpha=0.7) +
      geom_point(color="black", alpha=0.7) +
      scale_fill_manual(values = c("darkturquoise", "tomato")) +
      theme_bw() + labs(x="", y="Normalized expression", fill="") + 
      ggtitle(L1, subtitle = i) +
      stat_pvalue_manual(as.data.frame(padj_L1s_split[[L1]]), y.position = max(group_candidate_te_counts_norm_melt$value)+0.2*sd(group_candidate_te_counts_norm_melt$value), label = "padj") +
      lims(y = c(0,  max(group_candidate_te_counts_norm_melt$value)+0.4*sd(group_candidate_te_counts_norm_melt$value)))
    
  }
}

pdf("/Volumes/MyPassport/CRISPRi_L1s/results/plots/candidates_boxplots_L1s.pdf", height = 8, width = 10)
ggarrange(plotlist = candidate_L1_boxplots$hiPS6_crispri_g1)
# ggarrange(plotlist = candidate_L1_boxplots$hiPS6_crispri_g2)
ggarrange(plotlist = candidate_L1_boxplots$hiPS6_crispri_g3)
ggarrange(plotlist = candidate_L1_boxplots$hiPS48_crispri_g3)
ggarrange(plotlist = candidate_L1_boxplots$NES_crispra_g1)
# ggarrange(plotlist = candidate_L1_boxplots$NES_crispra_g2)
ggarrange(plotlist = candidate_L1_boxplots$NES_crispra_g3)
ggarrange(plotlist = candidate_L1_boxplots$hiPS6_org_crispri_g1)
# ggarrange(plotlist = candidate_L1_boxplots$hiPS6_org_crispri_g2)
ggarrange(plotlist = candidate_L1_boxplots$hiPS6_org_crispri_g3)
ggarrange(plotlist = candidate_L1_boxplots$hiPS6_org_crispri_g3_batch2)
ggarrange(plotlist = candidate_L1_boxplots$hiPS48_org_crispri_g3)
ggarrange(plotlist = candidate_L1_boxplots$hiPS6_org_crispra_g1)
# ggarrange(plotlist = candidate_L1_boxplots$hiPS6_org_crispra_g2)
ggarrange(plotlist = candidate_L1_boxplots$hiPS6_org_crispra_g3)
ggarrange(plotlist = candidate_L1_boxplots$hiPS6_org_crispra_g3_batch2)
dev.off()
