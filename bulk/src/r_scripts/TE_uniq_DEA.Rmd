...
### Genic vs Intergenic
```{r}
expressed_L1_protein_genes <- read.table("/Volumes/MyPassport/CRISPRi_L1s/bulk/results/tables/FL_L1PA_WT_hiPSC_expressed_intersect_protein_genes.bed")
...
FL_L1HS_L1PA4_id_norm_WT_hiPSC_expressed_bed <- merge(FL_L1HS_L1PA4_id_norm_WT_hiPSC_expressed_bed, unique(expressed_L1_protein_genes[,c("TE_id", "orientation"), drop=F]), by.x="Geneid", by.y="TE_id", all.x=T)

# pdf("/Volumes/MyPassport/CRISPRi_L1s/bulk/results/plots/expressed_L1HS_L1PA4_genic_orientation_FL_L1_WT_iPSC.pdf", height = 4, width = 6)
ggplot(FL_L1HS_L1PA4_id_norm_WT_hiPSC_expressed_bed, aes(x=TE_subfamily, fill=orientation)) + geom_histogram(stat="count") + theme_bw() + theme(text = element_text(size=15), panel.grid.minor = element_blank(), panel.grid.major = element_blank(), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))  + ggtitle("WT iPSC: Expressed FL L1HS-L1PA4\nGenic / intergenic") + labs(x="") + lims(y=c(0, 1000)) + facet_wrap(.~genic)
# dev.off()
```
