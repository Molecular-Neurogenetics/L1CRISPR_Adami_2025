...
samplesheet <- fread("/Volumes/MyPassport/CRISPRi_L1s/10X/metadata.tab", data.table = F)
...
samplesheet_g3 <- samplesheet[which(samplesheet$guide %in% c("gRNA3", "LacZ")),]
...
for(i in as.character(unique(samplesheet_g3$cluster_number))){
  ...
  gene_res[[i]] <- lfcShrink(gene_dds[[i]], "guide_gRNA3_vs_LacZ")
...
openxlsx::write.xlsx(sizeFactors, "/Volumes/MyPassport/CRISPRi_L1s/10X/results/tables/sizeFactors_pseudobulk.xlsx")
