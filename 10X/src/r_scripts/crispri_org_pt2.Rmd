...
index_reorder <- cluster_rows_by_group(df_cluster = df, group_rows = c("seurat_clusters", "condition"), samplesheet = upreg_genes_df[,c("seurat_clusters", "condition"), drop=F])

upreg_genes_df_col_annot <- upreg_genes_df[colnames(df[upreg_both,index_reorder]),c("condition", "seurat_clusters"),drop=F]
upreg_genes_df_col_annot$condition_cluster <- paste(upreg_genes_df_col_annot$seurat_clusters, upreg_genes_df_col_annot$condition, sep="_")
upreg_genes_df_col_annot_split <- split(upreg_genes_df_col_annot, f = c(upreg_genes_df_col_annot$condition_cluster))

index_reorder <- rownames(rbind(upreg_genes_df_col_annot_split$`0_LacZ`,
      upreg_genes_df_col_annot_split$`0_gRNA1`,
      upreg_genes_df_col_annot_split$`0_gRNA2`,
      upreg_genes_df_col_annot_split$`1_LacZ`,
      upreg_genes_df_col_annot_split$`1_gRNA1`,
      upreg_genes_df_col_annot_split$`1_gRNA2`,
      upreg_genes_df_col_annot_split$`4_LacZ`,
      upreg_genes_df_col_annot_split$`4_gRNA1`,
      upreg_genes_df_col_annot_split$`4_gRNA2`))

gaps <- rep((which(!duplicated(upreg_genes_df[colnames(df[upreg_both,index_reorder]),c("condition", "seurat_clusters"),drop=F]))[-1]-1), 3)

# png("~/Desktop/upreg_genes_crispri_org.png", units = "in", width = 14, height = 7, res=300) 
# pdf("~/Desktop/upreg_genes_crispri_org.pdf", width = 14, height = 7) 
pheatmap(df[upreg_both,index_reorder], annotation_col = upreg_genes_df[colnames(df[upreg_both,index_reorder]),c("condition", "seurat_clusters", "cell_line"),drop=F], border_color = NA, show_colnames = F, cluster_cols = F, scale="row", color = colorRampPalette(c("blue", "white", "red"))(100), breaks = seq(-4,4,length.out=100), gaps_col = gaps)
# dev.off()
```
